...
| [`JSUndefined`](https://github.com/cangjielanguage/cangjiecorpus/blob/1.0.0/ohos/zh-cn/application-dev/reference/arkinterop/cj-apis-ark_interop.md#class-jsundefined) | `Extern<ArkTS>` | Current interop uses a dedicated wrapper produced by `JSContext.undefined()`. With `Extern<ArkTS>`, the runtime would likely represent ArkTS `undefined` as an ordinary static function `ArkTS.undefined()`. |
| [`JSNull`](https://github.com/cangjielanguage/cangjiecorpus/blob/1.0.0/ohos/zh-cn/application-dev/reference/arkinterop/cj-apis-ark_interop.md#class-jsnull) | `Extern<ArkTS>` | Current interop uses a dedicated wrapper produced by `JSContext.null()`. The `Extern` model treats this as another foreign value handled through runtime conversion rules. |
| [`JSBoolean`](https://github.com/cangjielanguage/cangjiecorpus/blob/1.0.0/ohos/zh-cn/application-dev/reference/arkinterop/cj-apis-ark_interop.md#class-jsboolean) | `Extern<ArkTS>` | Current interop requires explicit wrapper calls such as `toBool()` and `toJSValue()`. The `Extern` model describes forced casts and implicit conversions through `fromExtern` and `toExtern`. |
| [`JSNumber`](https://github.com/cangjielanguage/cangjiecorpus/blob/1.0.0/ohos/zh-cn/application-dev/reference/arkinterop/cj-apis-ark_interop.md#class-jsnumber) | `Extern<ArkTS>` | Current interop uses `JSNumber` plus methods such as `toFloat64()`. `Extern<ArkTS>` would rely on runtime-provided conversion instead of a separate number wrapper. |
| [`JSString`](https://github.com/cangjielanguage/cangjiecorpus/blob/1.0.0/ohos/zh-cn/application-dev/reference/arkinterop/cj-apis-ark_interop.md#class-jsstring) | `Extern<ArkTS>` | Current interop uses a safe ArkTS string reference and explicit `toString()` or `toJSValue()`. The `Extern` model would expose this through casts/conversions on `Extern<ArkTS>`. |
| [`JSBigInt`](https://github.com/cangjielanguage/cangjiecorpus/blob/1.0.0/ohos/zh-cn/application-dev/reference/arkinterop/cj-apis-ark_interop.md#class-jsbigint) | `Extern<ArkTS>` | Current interop has a specialized safe reference for ArkTS `bigint`. In the `Extern` model, the runtime would represent this as a foreign value plus conversion support. |
